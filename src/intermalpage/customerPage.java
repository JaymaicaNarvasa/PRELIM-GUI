...
public class customerPage extends javax.swing.JInternalFrame {
    ...
    private void initComponents() {
        ...
        jPanel1.add(jLabel12, new org.netbeans.lib.awtextra.AbsoluteConstraints(450, -20, 260, 50));

        jLabel13.setFont(new java.awt.Font("Tahoma", 1, 18)); // NOI18N
        jLabel13.setText("________________________");
        jPanel1.add(jLabel13, new org.netbeans.lib.awtextra.AbsoluteConstraints(0, -30, 270, 70));
    ...
    private void delpaneMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_delpaneMouseClicked
        ...
        if(rowIndex < 0){
           ...
           if(a == JOptionPane.YES_OPTION){
               ...
               dbc.deleteData(cId, "tbl_customer", "c_id");
               displayData();
           }
        }
    ...
    private void editpaneMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_editpaneMouseClicked
        ...
        if(rowindex < 0){
            ...
            cf.gender = model.getValueAt(rowindex, 3).toString();
            
            String gen = model.getValueAt(rowindex, 3).toString();
            
            if(gen.equals("Male")){
                cf.male.setSelected(true);
            }
            
            if(gen.equals("Female")){
                cf.female.setSelected(true);
            }
            
            
            cf.address.setText(model.getValueAt(rowindex, 4).toString());
            cf.contact.setText(model.getValueAt(rowindex, 5).toString());
            cf.email.setText(model.getValueAt(rowindex, 6).toString());
            cf.status.setSelectedItem(model.getValueAt(rowindex, 7).toString());
            cf.setVisible(true);
            cf.action = "UPDATE";
            cf.label.setText("UPDATE");
            JFrame mainJFrame = (JFrame) SwingUtilities.getWindowAncestor(this);
            mainJFrame.dispose();
        }
